...
int main(int argc, char **argv) {
    ...
    // Input size per DPU (max.)
     unsigned int input_size_dpu = divceil(input_size, nr_of_dpus);                                                    
    ...
    //input_size_8bytes = 64;
    //input_size_dpu = 16;
    //input_size_dpu_8bytes = 16;
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        //DPU_ASSERT(dpu_broadcast_to( dpu_set, DPU_MRAM_HEAP_POINTER_NAME, 0, bufferL, input_size_8bytes * sizeof(T), DPU_XFER_DEFAULT));
        //DPU_ASSERT(dpu_broadcast_to( dpu_set, DPU_MRAM_HEAP_POINTER_NAME, 0, bufferU, input_size_8bytes * sizeof(T), DPU_XFER_DEFAULT));
